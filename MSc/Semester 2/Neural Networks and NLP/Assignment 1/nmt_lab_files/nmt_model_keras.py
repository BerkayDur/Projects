...
class NmtModel(object):
  ...
  def build(self):
    ...
    embeddings_source = Embedding(self.vocab_source_size, self.embedding_size, mask_zero=True)
    embeddings_target = Embedding(self.vocab_target_size, self.embedding_size, mask_zero=True)
    ...
    source_words_embeddings = Dropout(rate = self.embedding_dropout_rate)(embeddings_source(source_words))

    target_words_embeddings = Dropout(rate = self.embedding_dropout_rate)(embeddings_target(target_words))
...
def main(source_path, target_path, use_attention, use_tpu=False):
  max_example = 30000
